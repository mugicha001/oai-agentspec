...
- 受け入れ基準:
  ...
  - [ ] IF `on_trip="reject"` または `"allow"` を選ぶ THEN 検知後もモデルが当該データを読み得るため fail-closed ではないことを明記し、監査ログ収集など検知結果を続行前提で扱う用途に限る旨を記述する
