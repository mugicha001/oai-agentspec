...
str 以外の要素」「ツール境界の guardrail 名（振り分け先が存在しない）」の 4 種で、エージェント参照の群とは
別の接頭辞を持つ行として同一メッセージへ連結する。`guardrails` を宣言していない構成では検証結果・例外型・
メッセージ書式は変わらない。str 以外の要素については宣言リスト内の位置を問題行へ添える（同一型の不正要素が
複数あると型名だけでは要素を特定できないため）。

解決元が申告する境界が値域外（4 値のいずれでもない・そもそも str でない）の場合は**集約対象に含めない**。
値域は解決元側の責務であり、この不整合は build 時に例外で落ちる。したがって「`validate()` は成功するが build で
落ちる」ケースが 2 種ある（申告境界が値域外・申告境界と実体境界の不一致）。同一の参照について逆方向
（`validate()` が落ちるのに build が通る）は存在しない。ただし `validate()` は全 spec を走査するのに対し build は
要求された Agent の依存閉包のみを構築するため、閉包外の spec が持つ問題は build に現れない（エージェント参照と
共通の性質）。
...
`route_match` / `last_agent_match` / `approval_match` / `judge`）・`prompt_slot` /
...
- **APO（`algorithm="apo"`・`[lightning]` のみで完結）**: 最適化対象は利用者指定のプロンプトスロット（vars 未展開
  ...
  （空 / `Slot.name` の重複 / 非 `Slot` の混在は fail-closed）。`prompt_slot`（`prompt_slot_factory` は
  これへ委譲する）は `PromptStore` の公開 `compose` / `get` を読み取り seed と固定部分を再合成し、`build`（候補テキスト →
...
ポリシー強制）は直交する役割分担であり、相互に置き換えない。

宣言経路は `AgentSpec` の専用フィールドと名前参照の 2 つである。`input_guardrails` / `output_guardrails` は
`agents.Agent` の正規フィールドを鏡写しにした専用フィールドで、ファクトリが返した guardrail オブジェクトを
そのまま渡す。`guardrails` は登録簿へ登録した guardrail を**名前で**参照する専用フィールドで、`handoffs` /
`sub_agents` と同じ流儀である。いずれも `AgentSpec.extra` での素通しは行わない（`extra` に同名キーを渡すと
専用フィールドとの衝突として `ValueError` になる）。本支援層は SDK 互換 guardrail を生成するファクトリ群と、
それらを名前・境界・分類・危険度の宣言として保持する登録簿を提供する。
...
guardrail を「名前・適用境界・framework ラベル・危険度」の宣言として保持する登録簿を持つ。宣言経路は 3 つで、
いずれも併存する。

| 経路 | 実体を作るのは | 名前を持つか | 使いどころ |
|---|---|---|---|
| facade 経路 | 登録簿（同梱ファクトリを代理呼び出し） | 持つ | 同梱 helper を名前参照する既定経路 |
| register 経路 | 利用者 | 持つ | 生の SDK guardrail・自作の実体を名前参照へ載せる逃げ道 |
| オブジェクト直接渡し | 利用者 | 持たない | 名前参照が不要な場合（従来経路・変更なし） |

**facade 経路は生成と登録を同一呼び出しで行う**。登録キーを `name` としてファクトリへ注入するため、登録キーと
上流 SDK 可視名（`get_name()` の戻り値）の一致が構造的に成立し、事後の名前差し替えを持たない。同時に登録簿が
どの helper 由来かを知るため、helper 自体で分類が固定される helper には framework ラベルと既定危険度を自動
付与する（利用者が明示した値が優先する）。register 経路は登録時に「上流 guardrail 型か」「可視名が登録キーと
一致するか」「宣言境界と実体の境界が一致するか」を検証し、自動付与は行わない。

境界の値域は上流 SDK の guardrail 型 4 種と 1 対 1 に対応する（`input` / `output` / `tool_input` /
`tool_output`）。値域型は文字列リテラルと等価に扱える列挙で、照会の戻り値は列挙メンバへ正規化される。危険度は
順序を持つ有限値域（`low` < `medium` < `high` < `critical`・未宣言は非比較）で、順序は比較演算子で成立する。

適用範囲は 2 つあり、上流 SDK は両者を連結して評価する。入力側は起動エージェントの分と run 単位の分を連結して
最初のターンでのみ評価し、出力側はその時点のエージェントの分と run 単位の分を連結する。したがって **Agent 単位の
出力 guardrail はハンドオフ先に付いていなければ評価されない**。ハンドオフ先を含む run 全体へ一律に掛けたい
guardrail は run 単位（`RunConfig`）で渡す。ツール境界の guardrail は `agents.Agent` にも `RunConfig` にも
フィールドが存在しないため名前参照の対象外で、登録簿から取り出して `function_tool(tool_input_guardrails=...,
tool_output_guardrails=...)` へ渡す（名前参照へ渡した場合は無言で落とさず `ValueError`）。

run 単位へ束ねる照会は、境界別の振り分けを利用者に書かせず `RunConfig` の引数名をキーとするマッピングを返す。
検証は「要素の型 → 名前の解決 → 境界の受け入れ可否」の順に各段で全要素を一巡させる（複数の違反が混在しても
送出される例外型が入力順に依存しない）。対象が空でもキーを欠落させず、対象名を省略した呼び出しは登録全件を
名前昇順で対象とする。**全件にツール境界の登録が 1 件でも含まれる場合は静かに除外せず例外**とする（除外すると
「掛けたつもりの検査が無い run」が無言で成立する）。ツール境界を含む登録簿から run 単位の束を得たい場合は、
agent 境界の名前を明示する。

`AgentRegistry` は登録簿の実装型を参照せず、`protocols.py` の Protocol（「名前 → 実体」「名前 → 宣言境界の
str」の 2 照会）でのみ受け取る（キーワード専用引数・既定は未注入で従来挙動）。Tool メタデータの登録簿
（「Tool Registry」節）が `AgentSpec` / `AgentRegistry` から独立で注入点を持たないのに対し、本登録簿は名前参照を
成立させるために注入点を持つという非対称がある。判断の経緯は `docs/adr/0015-declarative-guardrail-registry.md`
を参照する。

Protocol は duck-typed で、解決元が申告する境界の値域も実体の型も検証しない。そのため build 時に「実体から
判定した境界」と「申告された境界」を突き合わせ、食い違えば例外にする。突合は**実体が上流 4 型と判定できた
ときに限る**（Protocol の契約は「不透明型を返す」であり、判定不能を不一致として扱うと契約を実質「上流 4 型を
返す」へ狭めてしまう）。申告された境界が agent 境界でない場合は既定境界へフォールバックせず例外にする。

### 名前参照の解決境界

名前参照の解決は **`AgentRegistry` 経由の build に限る**。解決元は `AgentRegistry` が保持するため、registry を
通らない build 経路では名前参照を解決できない。評価（LLMOps）・最適化（Agent Lightning）が単体の `AgentSpec` を
対象に取る経路は registry を経由しないため、`guardrails` を宣言した spec を渡した場合は**明示的な例外で拒否**
する（無言で落とすと「宣言した検査が存在しない対象」を測ってしまう）。専用フィールドへ実体を渡す経路は
builder が転送するため同経路でも適用される。横断対象（`HandoffGraph` / `WorkflowGraph`）は利用者 registry を
経由するため影響を受けない。
...
- **名前参照は `handoffs` を鏡写し**: 登録簿へ登録した guardrail は `AgentSpec.guardrails`（`list[str]`・
  キーワード専用）へ名前で宣言する。境界は宣言から解決されるため利用者は境界を再指定しない。専用フィールドと
  併記した場合、各境界のリストは「専用フィールドの既存要素 → 名前参照由来の宣言順」で連結し、重複排除は行わない。
- **登録簿の照会はメソッド**（属性アクセスは採らない）。Tool メタデータの登録簿が属性アクセス（`registry.<name>`）を
  採るのに対し、guardrail 名は `AgentSpec.guardrails` の文字列として宣言されるため、属性到達性（識別子であること・
  公開メソッド名と衝突しないこと）の制約を名前に課さない。個々の名前の検証は「str であり、空文字でなく、空白のみで
  ない」に限る。ただし `AgentSpec.guardrails` は**コンテナの型も検証**する（素の str は 1 文字ずつ反復されるため
  宣言の時点で拒否する）。
...
取得系データ源（RAG / メモリ / ツール出力）の改竄検知を `Boundary.TOOL_OUTPUT` で行うパターンは
`docs/integrity.md` の「データ源インテグリティ（RAG / メモリ / ツール出力）」節を参照する。
...
- `enable_agent365_tracing(config)`: MS 拡張 `configure()` をラップし、その後に MS 拡張の公式 API
  ...
  読み、あらゆる例外を吸収する（利用者が渡す不透明値の属性アクセスで有効化を失敗させない）。吸収した
  例外は警告せず DEBUG ログへトレースバックと options の型を記録する（無音で捨てると番兵へ倒れた事実が
  判定対象外の構成と外形上区別できないため）。`isinstance` による型判別は行わないため sidecar 構成
  （`SpectraExporterOptions`・当該属性を持たない）は番兵で除外される。有効化フラグ未設定に起因する
  未達は検知しない（本層は環境変数を読まない）。同じ合成規則で無視される `cluster_category` も検知
  対象に含めない（既定へ倒れるだけで認証手段は失われないため。docs で開示する）。詳細は
  `docs/adr/0024-agent365-export-target-detection-scope.md`。
