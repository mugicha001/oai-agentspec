...
`dpo_dataset_from_session`）・検証（`validate_dataset`）・ツール往復のスクリーニング
（`screen_tool_roundtrips`）・投入前の仕分け（`partition_dataset`）・DPO 雛形の記入ワークフロー
...
`to_dpo_dataset` / `validate_dataset` / `screen_tool_roundtrips` / `partition_dataset`）と雛形ヘルパ
（`save_dpo_draft` /
