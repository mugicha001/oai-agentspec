...
class Agent365TracingConfig:
    ...
    Attributes:
        ...
        cluster_category: MS 拡張のクラスタ区分（既定 "prod"）。`token_resolver` と同じ合成規則で、
            `exporter_options` を渡した場合は本フィールドが参照されない。Agent365 形式の options
            では options 側の `cluster_category`（既定 "prod"）が使われるため非既定のクラスタへ送る
            ときはそちらへ設定し、sidecar 向け options ではクラスタ区分自体が使われない。
        exporter_options: sidecar 向け `SpectraExporterOptions` 等の不透明値。lib は素通しを原則と
            する（SDK 隔離）。例外として `token_resolver` 属性の有無と値のみを警告判定に読む
            （型判別もエクスポータ選択も行わない。ADR 0024）。この読み取りは有効化時に同期で
            1 回だけ行うため、ブロッキングな計算 property を本値に持たせない。読み取りが例外に
            なった場合、その例外はトレースバック付きで DEBUG ログ（logger 名
            `oai_agentspec._adapters.observability`）へ記録されるため、例外メッセージへトークン等の
            機微情報を含めない。
