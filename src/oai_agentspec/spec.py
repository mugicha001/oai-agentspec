...
class AgentSpec:
    ...
    Attributes:
        ...
        mcp_config: `Agent.mcp_config`。MCP 設定（`convert_schemas_to_strict` /
            ...
            真にするとツールの公開名が `mcp_{サーバ名}__{ツール名}` を基本形とする名前になるため、
            名前でツールを参照する仕組み（ガバナンスの `allowed_tools` 等）を併用している場合は
            追随が必要。サーバ名部分 / ツール名部分はそれぞれ ASCII 英数字 / `_` / `-` 以外の文字が
            `_` へ置換され、前後の `_-` が strip される（strip 後に空になると `server` / `tool` へ
            フォールバックする）。基本形が長さ上限を超える場合は切り詰めてハッシュを付け、
            `spec.tools` / handoff / as_tool のツール名や同一解決バッチ内の他ツールと衝突する場合は
            上限以内でもハッシュが付く。基本形をそのまま宣言すると不一致になりうるため、実際の
            公開名を確認して宣言する。
